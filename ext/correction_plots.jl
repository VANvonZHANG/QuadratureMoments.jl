function plot_moment_comparison(
    ...
    if colors !== nothing && length(colors) < n_sets
        throw(
            ArgumentError(
                "colors vector length ($(length(colors))) < number of sets ($n_sets)"
            ),
        )
    end
    if labels !== nothing && length(labels) < n_sets
        throw(
            ArgumentError(
                "labels vector length ($(length(labels))) < number of sets ($n_sets)"
            ),
        )
    end
    ...
    for (i, ms) in enumerate(moment_sets)
        length(ms) == n_moments || throw(
            ArgumentError("Moment set $i has $(length(ms)) elements, expected $n_moments"),
        )
    end
