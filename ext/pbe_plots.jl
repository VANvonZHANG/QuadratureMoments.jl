function plot_moment_evolution(
    ...
    if colors !== nothing && length(colors) < n_mom
        throw(
            ArgumentError(
                "colors vector length ($(length(colors))) < number of moments ($n_mom)"
            ),
        )
    end
    if labels !== nothing && length(labels) < n_mom
        throw(
            ArgumentError(
                "labels vector length ($(length(labels))) < number of moments ($n_mom)"
            ),
        )
    end
...
function plot_ndf_snapshots(
    ...
    if colors !== nothing && length(colors) < n_snaps
        throw(
            ArgumentError(
                "colors vector length ($(length(colors))) < number of snapshots ($n_snaps)"
            ),
        )
    end
...
function plot_pbe_summary(
    ...
    p1 = plot_moment_evolution(
        t_dense, moments; exact=exact, labels=labels, colors=colors, title=title1
    )
