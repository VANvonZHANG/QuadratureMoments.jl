...
N-node Extended Quadrature-Based Moment Method with kernel K.
"""
...
"""
function compute_modified_moments(m::SVector{L, T}, σ::T, ::GaussianKernel, ::AbstractMathBackend) where {L, T}
    ...
    for k in 3:L # Index 1=m0, 2=m1, 3=m2
        ...
        for j in 1:floor(Int, k_val/2)
            c = one(T)
            for m_idx in 0:(2j-1) c *= (k_val - m_idx) end
            for m_idx in 1:j c /= (2*m_idx) end
