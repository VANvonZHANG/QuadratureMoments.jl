...
suite["Wheeler Inversion"]["Static Array"] = @benchmarkable wheeler_inversion(
    $m_1d_static_6
)
suite["PD Inversion"]["Static Array"] = @benchmarkable pd_inversion($m_1d_static_6)
suite["EQMOM (Gaussian)"]["Static Array"] = @benchmarkable invert_moments(
    $(EQMOM(3, GaussianKernel())), $m_1d_static_7
)
...
suite["CQMOM (2D)"]["Recursive Static"] = @benchmarkable invert_moments(
    $(CQMOM((2, 2))), $m_4x4
)
suite["ECQMOM (2D)"]["Recursive Static"] = @benchmarkable invert_moments(
    $(ECQMOM((2, 1))), $(SMatrix{5,3,Float64}(m_5x5[1:5, 1:3]))
)
suite["TensorQMOM (2D)"]["Static"] = @benchmarkable invert_moments(
    $(TensorQMOM((2, 2))), $m_4x4
)
...
suite["DQMOM Solve"]["Static Array"] = @benchmarkable dqmom_solve(
    $dq_nodes_static, $dq_source_static
)
